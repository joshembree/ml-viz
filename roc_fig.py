...
def roc_plot_data(df, label='label', score='score', split_column=None):
...
def make_visible_dict(plot_dfs_list, split_cols, version):
...
for version in versions:
    plot_dfs[version] = {}
    for split_col in split_cols:
        plot_dfs[version][split_col] = roc_plot_data(df2[df2['model'] == version], split_column=split_col)
        plot_dfs[version][split_col] = plot_dfs[version][split_col].round(4)

version = versions[0]
visible = make_visible_dict(plot_dfs, split_cols, version)
